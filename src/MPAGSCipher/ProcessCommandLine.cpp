                        ...
                        ProgramSettings& settings)
{
    ...
    for (std::size_t i{1}; i < nCmdLineArgs; ++i) {
        if (cmdLineArgs[i] == "-h" || cmdLineArgs[i] == "--help") {
            ...
            if (i == nCmdLineArgs - 1) {
                throw MissingArgument("-i requires an argument");
            } else {
            ...
            if (i == nCmdLineArgs - 1) {
                throw MissingArgument("-o requires an argument");
            } else {
                // Got filename, so assign value and advance past it
            ...
            if (i == nCmdLineArgs - 1) {
                throw MissingArgument("-k requires an argument");
            } else {
            ...
            if (i == nCmdLineArgs - 1) {
                throw MissingArgument("-c requires an arguments");
            } else {
                ...
                if (cmdLineArgs[i + 1] == "caesar") {
                    ...
                    throw UnknownArgument("Given cipher argument was not recognised");
                }
                ++i;
            }
        } else {
            throw UnknownArgument("Encountered an unknown argument: " + cmdLineArgs[i]);
        }
    }
